...
def rotate_tool_z(planner, theta_rad, accel=0.5, vel=0.2):
    script = String()
    script.data = (
        "def rot_z():\n"
        f"  rot = p[0, 0, 0, 0, 0, {theta_rad}]\n"
        "  target = pose_trans(get_actual_tcp_pose(), rot)\n"
        f"  movel(target, a={accel}, v={vel})\n"
        "end\n"
    )
    planner._urscript_pub.publish(script)
    planner.get_logger().info(f"Sent tool-Z rotation: {theta_rad} rad")
...
def main():
    ...
    right_target.pose.position.x = -0.32
    right_target.pose.position.y = -0.7
    right_target.pose.position.z = 0.32
    ...
    planner._add_box_obstacle("wall1", -0.4, right_target.pose.position.y + 0.15, 0.5, 0.2, 0.01, 0.8, frame="right_base")
    ...
    if ok:
        ...
        time.sleep(1.0)
        rotate_tool_z(planner, math.radians(30))
