...
# Eye-in-hand calibration for the RIGHT arm's wrist D405.
#
...
# serial 130322274012 (commented "left wrist D405 (robot2)") is actually the
# RIGHT arm's wrist camera — that's the one this file calibrates.
#
# Still verify before running:
#   1. The wrist camera's actual optical TF frame id — run
#      `ros2 topic echo /camera/camera/color/camera_info --field header.frame_id`
...
def generate_launch_description():
    ...
    tracking_base_frame_arg = DeclareLaunchArgument(
        ...
        description=(
            "Wrist D405 (right arm) optical TF frame. VERIFY this against the "
            ...
            "with (see real_robot_bringup.launch.py's camera_node)."
        ),
    )
